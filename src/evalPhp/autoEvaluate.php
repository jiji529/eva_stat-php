...
function autoEvaluate($db, $config_eval, $news_id_arr, $premiumID) {
        ...
        if (!is_array($news_id_arr) || count($news_id_arr) === 0) {
          ...
          foreach($news_id_arr as $news_id) {
            ...
            if ($news_id_arr_count > 10000 || $news_id_arr_count_acm >= $news_id_arr_entire) {
              ...
              $query_fetch .= "WHERE (`eval2`.`evaluation_seq` <= 1000 OR `eval2`.`evaluation_seq` IS NULL) ";
              $query_fetch .= "AND `hnp_news`.`news_id` IN " . $news_id_str . " ";
              $query_fetch .= "GROUP BY `hnp_news`.`news_id` ORDER BY `hnp_news`.`news_id`) ";
              $query_fetch .= "`B` WHERE `B`.`eval_count` = 1 AND `B`.`evaluation_seq` IS NULL";
              ...
              if ($db->Error()) {
                ...
                while ($row = mysqli_fetch_assoc($db->Records())) {
                  ...
                  // if (equals($config_eval['policy']['AT'][2]['value'], 'Y')) {
                  ...
                  // if (equals($config_eval['policy']['AT'][3]['value'], 'Y')) {
                  $tmp_fetch_seq = end($sorted_gi_config[3]);
                  $tmp_fetch_seq = $tmp_fetch_seq['seq'];
                  ...
                  // if (equals($config_eval['policy']['AT'][4]['value'], 'Y')) {
                  ...
                  // if (equals($config_eval['policy']['AT'][5]['value'], 'Y')) {
